...
static void readNetworkFile(char *filename, struct test *t) {
	...
	} else if (t->numComputers <= 0) {
		errx(EXIT_FAILURE, "error: invalid number of computers");
	}

	if (fscanf(fp, "%d", &(t->numConnections)) != 1) {
		errx(EXIT_FAILURE, "error: failed to read number of connections");
	} else if (t->numConnections < 0) {
		errx(EXIT_FAILURE, "error: invalid number of connections");
	}
	...
	for (int i = 0; i < t->numConnections; i++) {
		...
		if (fscanf(fp, "%d %d %d", &compA, &compB, &transmissionTime) != 3) {
			...
			errx(EXIT_FAILURE, "error: invalid self-connection");
...
static void scanProbePath(FILE *fp, struct test *t) {
	...
	} else if (t->pathLength <= 0) {
		errx(EXIT_FAILURE, "error: invalid path length");
	}
	...
	for (int i = 0; i < t->pathLength; i++) {
		...
		} else if (t->path[i] < 0 || t->path[i] >= t->numComputers) {
			errx(EXIT_FAILURE, "error: invalid computer number '%d'",
			     t->path[i]);
		}
	}
}
